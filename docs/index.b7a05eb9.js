...
const dbRef = _database.ref(_firebaseapp.db, '/chatt');
let messages = [];
_database.onValue(dbRef, (snapshot)=>{
    const messageData = snapshot.val();
    console.log(messageData);
    for (const message of messages)message.clearDOM();
    messages = [];
    for(const key in messageData)messages.push(new _message.Message(key, messageData[key].name, messageData[key].message));
    divdown();
    function remove26() {
        ...
        if (messageArray.length > 25) {
            ...
            const test = _database.ref(_firebaseapp.db, '/chatt/' + index0);
            _database.remove(test);
        } else console.log(messageArray.length);
    }
    remove26();
});
document.querySelector('#send').addEventListener('click', (e)=>{
    ...
    _database.update(dbRef, newMessage);
});
function divdown() {
...
},{"firebase/database":"bpqHw","./modules/firebaseapp":"aqtxR","./modules/Message":"kpGuV"}],"bpqHw":[function(require,module,exports) {
...
},{"@firebase/app":"3AcPV","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"kpGuV":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
    ...
    displayMsg() {
        ...
        removeBtn.addEventListener('click', ()=>{
            ...
            if (this.name == name.value) {
                const msgRef = _database.ref(_firebaseapp.db, '/chatt/' + this.id);
